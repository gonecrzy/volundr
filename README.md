...
provider, and proportional planning artifacts derived from the authoritative
requirement ledger.
...
```text
describe product
  -> extract and preserve requirements
  -> clarify only materially missing information
  -> select proportional planning depth
      - direct brief
      - compact plan
      - detailed plan
  -> normalize into GeometryExecutionContext
  -> generate safe CadQuery source
  -> execute in isolated CAD worker
  -> check geometry, topology, artifacts, and requirements
  -> promote a passing Current working version
  -> revise, reopen, compare, or export through chat
```
...
Plan, or detailed plan before generation. Passing generated revisions become the Current working
...
Projects can be renamed or archived from the browser workspace; archiving hides
projects from the default list while preserving them indefinitely for explicit
reopen, revision, reprint, and export. Delete is the explicit destructive
operation. Operators can use the documented dry-run archive purge command when
maintenance removal is genuinely required.
...
compact plans handle interacting single-part features, and detailed plans
remain available for multipart or assembly relationships. See
...
V1 multipart support covers simple component/output relationships and
printable-part packaging. True assembly mates, ports, kinematics, and
mechanism simulation remain later capabilities and are not implied by a
multi-component Design Plan.

### Nonblocking parametric opportunities

After a successful design, Volundr may later suggest reusable controls for
values such as repeated count, capacity, spacing, wall thickness, or clearance.
