...
def finalize_study(output_root: Path) -> dict[str, Any]:
    ...
    attempt_events = [{"phase": name, "logical_operation_id": record.get("logical_operation_id"), "provider_attempt_id": attempt.get("provider_attempt_id"), "event": attempt.get("limiter_event")} for name, report in phase_reports.items() for record in report.get("records", []) for attempt in record.get("attempts", [])]
    rate_report = {"schema_version": "gemini-provider-contract-foundation-rate-limit-report-v1", "study_id": STUDY_ID, "provider_calls": len(all_attempts), "worker_calls": 0, "policy": {"default_requests_per_minute": 12, "hard_max_requests_per_rolling_60_seconds": 15, "minimum_gap_seconds": 5, "concurrency": 1, "clock": "monotonic"}, "phase_summaries": {name: {"provider_calls": sum(len(item.get("attempts", [])) for item in report.get("records", [])), "logical_operations": len(report.get("records", [])), "complete_logical_operations": sum(bool(item.get("complete")) for item in report.get("records", [])), "recorded_limiter_events": sum(len(item.get("attempts", [])) for item in report.get("records", []))} for name, report in phase_reports.items()}, "events": attempt_events, "event_source": "immutable per-attempt limiter_event fields; resumed report arrays are not trusted"}
