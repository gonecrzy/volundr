...
def main() -> None:
    ...
    write("creation-workflow", {
        **common,
        "status": "simple_creation_package_passed_complex_creation_semantic_verification_blocked",
        ...
        "complex_creation_probe": {
            "workflow_id": "99913965-0171-4715-b03c-24b76f1068e3",
            "project_id": "f0f63a1d-b2b9-455f-be53-cbc0c9d9597c",
            "explicit_asymmetric_dimensions": {"width_mm": 120, "depth_mm": 70, "height_mm": 5},
            "requested_feature_types": ["mounting_holes", "cable_slot"],
            "requested_irregular_layout": True,
            "workflow_state": "failed",
            "worker_geometry": "valid single solid artifacts were produced",
            "application_semantic_verification": "failed; safe diagnostic reported output not generated",
            "package_available": False,
        },
        "required_complexity_gap": "The richer real creation reached valid worker geometry but failed application semantic verification; this is the remaining narrow application-owned boundary.",
        "clean_database_preserved": "no historical application rows restored; only controlled smoke rows exist",
    })
    write("revision-workflow", {
        **common,
        "status": "accepted_revision_and_package_passed",
        "real_staging_attempted": True,
        ...
        "successful_workflow_id": "ec677562-142d-4753-be2b-54f755ebdd67",
        "revision_id": "75a730b4-bc8b-49d6-95de-60e7c765cdf7",
        "terminal_state": "revision_ready",
        "revision_review_state": "accepted",
        "package_available": True,
        "protected_facts_preserved": ["primary printable output identity", "single rectangular spacer purpose", "source authority"],
        "artifact_download_statuses": {"step": 200, "stl": 200, "brep": 200, "design-package": 200},
        "deterministic_fixture_contract": "passed in test_validated_cadquery_workflow.py",
    })
    write("partial-output-workflow", {
        **common,
        "status": "fixture_replay_passed_real_multi_output_not_spent",
        ...
        "reason": "The real creation and revision paths are credentialed and passed where applicable; the multi-output sibling-failure case was replayed through the product service contract to avoid an unnecessary additional provider operation.",
    })
    write("restart-recovery", {
        **common,
        "status": "accepted_creation_and_revision_persisted_across_controlled_restarts_fixture_recovery_passed",
        ...
        "persisted_revision_workflow_id": "ec677562-142d-4753-be2b-54f755ebdd67",
    ...
    write("provider-attempts", {
        **common,
        ...
        "external_provider_attempt_count": 22,
        "workflow_associated_attempt_count": 19,
        "status_codes": [200],
        "credential_slot_counts": {"primary": 22, "fallback": 0},
    ...
    write("worker-jobs", {
        **common,
        ...
        "jobs_root": "data/jobs",
        "jobs_root_bytes_after_reconciliation": 713627,
        "real_workflow_job_count": 5,
    ...
    write("package-integrity", {
        **common,
        ...
        "real_package_count": 2,
        "workflow_ids": ["62bd12c3-e8a8-460f-8a1b-ec3e57e0cee3", "ec677562-142d-4753-be2b-54f755ebdd67"],
    ...
    write("final-database-decision", {
        **common,
        ...
        "provider_backed_product_flow": "credentialed simple and feature-rich creation probes, accepted creation, bounded revision, artifacts, packages, and restart persistence demonstrated",
        "blocker": "The feature-rich creation produced valid worker geometry but application semantic verification marked its required output as not generated; multi-output partial isolation is covered by replayed product contract rather than another provider spend.",
        "database_state": "clean migration baseline with controlled smoke rows only; no historical user design data restored",
        "next_required_fix": "Align feature-rich creation semantic verification with the valid worker output before declaring the full multi-feature baseline ready.",
        "secret_values_serialized": False,
    })
    ...
    combined = {
        **common,
        ...
        "database": {"target": "sqlite:///<redacted>/app.db", "row_policy": "migration started empty; only controlled smoke rows added afterward", "artifact_root_bytes": 713627},
        "completed": ["guarded reset", "base-to-head migration", "fresh schema audit", "0039 alignment and repeated rebuild", "artifact quarantine", "default and controlled routing checks", "root .env credential layering", "credentialed simple creation/acceptance", "credentialed bounded revision", "artifact/package integrity", "restart persistence", "fixture partial-output replay"],
        "blocked": ["full feature-rich creation semantic verification", "real multi-output sibling failure"],
        "blocker": "Feature-rich creation worker geometry is valid but the application semantic verifier records the required output as not generated; this is a narrow application-owned fix, not a database or credential blocker.",
