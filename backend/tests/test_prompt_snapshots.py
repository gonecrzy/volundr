...
def test_scaffold_prompt_requests_geometry_functions_only() -> None:
    ...
    assert "place_pattern_cutters" in prompt
    assert "Workplane.assembly()" in prompt
    assert provider.prompt_template_version_for(request) == "cadquery-geometry-body-v10"
...
def test_structured_geometry_body_repair_preserves_parameter_effect_manifest() -> None:
    ...
    assert provider.prompt_template_version_for(request) == "cadquery-geometry-body-repair-v10"
...
def test_requirement_prompt_is_json_only_and_clarification_capable() -> None:
    ...
    assert provider.requirement_prompt_template_version() == "requirements-v5"
...
def test_design_plan_prompt_is_json_only_and_product_model_aware() -> None:
    ...
    assert provider.design_plan_prompt_template_version() == "design-plan-v8"
    assert "Return JSON only. Do not generate CAD source." in prompt
