    ...
    def _build_requirement_prompt(self, request: RequirementExtractionRequest) -> str:
        if request.schema_repair_of_raw_output is not None:
            ...
            task = [
                ...
                "When one source fact contains multiple independently actionable semantic parts with different authority or verification meaning, emit them in semantic_parts with stable part IDs and semantic_role values, while retaining the parent record's source_fact_id and source_fact_evidence. Set independent=true only when the part is separately actionable. Keep delegated choice instructions separate from qualitative objectives and explicit fixed values. A user instruction to choose a value is not the same as a user-specified value: delegated choices must be flexible, explicit=false, and protected=false even though the delegation itself came from the user. If the semantic split is uncertain, do not invent semantic_parts.",
            ...
            "critical_dimensions": [
                {
                    ...
                    "semantic_parts": [
                        {
                            "id": "string",
                            "semantic_role": "delegated_choice|qualitative_objective|structural_intent|other",
                            "independent": True,
                            ...
                            "unit": "string|null",
                            "source": "user|clarification|calculated|product_default|ai_assumption|null",
                            "authority": "explicit|flexible|provisional|proposed|null",
                            "explicit": True,
                            "protected": False,
                            "delegated": False,
