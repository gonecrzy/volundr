...
class RevisionOutput(Base):
    ...
    brep_hash: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    ...
    expected_solid_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    detected_solid_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    allow_disconnected_solids: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
