...
- source-probe repair can now be enabled with `--source-probe-repair` to run one bounded compile-repair pass after a failed source-probe compile while keeping first-pass and repaired metrics separate
