...
| `VOLUNDR_EXECUTABLE_CADQUERY_CORPUS_MANIFEST_PATH` | API and Compose | unset | optional executable-route contract source | yes | no | Explicit repeatability/replay corpus manifest. Ordinary user and external-benchmark runs derive the contract from the persisted requirement ledger when this is unset. |
